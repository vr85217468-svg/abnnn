...
export default async function () {
    console.log(`[Sync] Starting background sync for video ${VIDEO_ID}...`);
    try {
        const messages = await getYouTubeChat();
        if (messages.length > 0) {
            const res = await go(`${SUPA_URL}/rest/v1/comments?on_conflict=youtube_id`, {
                method: "POST",
                headers: {
                    apikey: SUPA_KEY,
            ...
            console.log(`[Sync] Successfully synced ${messages.length} messages.`);
        } else {
            console.log("[Sync] No new messages found.");
        }
    } catch (e) {
        console.error("[Sync] Critical error during background sync:", e.message);
    }
}
