...
exports.handler = async function () {
  // 1. جلب التعليقات الحالية من يوتيوب (تعكس الحذف)
  let liveMessages = [];
  try { liveMessages = await getYouTubeChat(); } catch (_) { }

  // 2. حفظ في Supabase للأرشيف (الجديد يُحفظ، المكرر يُتجاهل، المحذوف يبقى)
  if (liveMessages.length > 0) {
    try {
      await go(`${SUPA_URL}/rest/v1/comments?on_conflict=youtube_id`, {
        ...
        body: JSON.stringify(liveMessages),
      });
    } catch (_) { }
  }

  // 3. إرجاع التعليقات الحالية فقط (ليس من Supabase)
  //    إذا حُذفت رسالة من يوتيوب، لن تظهر هنا
  //    لكنها تبقى في Supabase (الأرشيف)
  return {
    statusCode: 200,
